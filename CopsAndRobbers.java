...
public class CopsAndRobbers extends javax.swing.JApplet {

	static final String instructions = "<html>" + "<b><h2><center>Instructions for Cops and Robbers</center></h2></b>";
	...
	public CopsAndRobbers() {
		...
		VisualizationModel<String, Number> vm = new DefaultVisualizationModel<String, Number>(layout, preferredSize1);
		...
		JTextField copNumberField = new JTextField(2);
		copNumberField.setEditable(true);
		JLabel copNumberLabel = new JLabel("Cop Number: ");
		
		JButton startButton = new JButton("Start");
		
		
		JPanel gameControls = new JPanel();
		gameControls.setBorder(BorderFactory.createTitledBorder("Game Controls"));
		gameControls.add(copNumberLabel);
		gameControls.add(copNumberField);
		gameControls.add(startButton);
	...
	public static void main(String[] args) {
		JFrame f = new JFrame("Cops and Robbers v 0.1");
		Dimension appsize = new Dimension(800,600);	
		f.setSize(appsize);
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		f.getContentPane().add(new CopsAndRobbers());
		f.pack();
		f.setVisible(true);
	}
}